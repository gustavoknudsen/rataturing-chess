...
| file | covers | built |
|---|---|---|
| `rataturing.bin` | the starting positions the tournament used, to move 20 | rebuilt on finals day from the games played in the qualifier and the practice rounds; it covers everything the pre-qualifier book did and more |
| `rataturing_hedge.bin` | the standard start, as insurance for openings the main book misses | before the qualifier |

The engine consults the main book first and the hedge only on a miss. The main book is built outward from the tournament's starting positions, which is why it has no entry for the standard starting position; its moves come from our own engine labelling. The hedge fills gaps from public game data and an existing book.
