...
@njit(cache=False, fastmath=True)
def evaluate(bb, st):
    """Material + piece-square tables + tempo on BTC's unified scale.
    Phase 3 extends this with the full hand-crafted evaluation."""
...
@njit(cache=False, fastmath=True)
def _score_move(bb, st, mv, ply, tt_move, killers, main_hist):
    if tt_move != 0 and mv == tt_move:
        return SCORE_TT_BEST
    if mv & (1 << 20):
        captured = _captured_piece(bb, st, mv)
        attacker = (mv & 0xF000) >> 12
        return SCORE_GOOD_CAPTURE + MVV_LVA[attacker, captured] * 100
    if killers[0, ply] == mv:
        return SCORE_KILLER_1
    if killers[1, ply] == mv:
        return SCORE_KILLER_2
    return int(main_hist[(mv & 0xF000) >> 12, (mv & 0xFC0) >> 6])
...
def _sort_moves(bb, st, ml, scores, cnt, ply, tt_move, killers, main_hist):
    for i in range(cnt):
        scores[i] = _score_move(bb, st, ml[i], ply, tt_move, killers, main_hist)
    _insertion_sort(ml, scores, cnt)


@njit(cache=False, fastmath=True)
def _sort_captures(bb, st, ml, scores, cnt):
    for i in range(cnt):
        mv = ml[i]
        captured = _captured_piece(bb, st, mv)
        attacker = (mv & 0xF000) >> 12
        scores[i] = MVV_LVA[attacker, captured]
    _insertion_sort(ml, scores, cnt)
...
@njit(cache=False, fastmath=True)
def qsearch(alpha, beta, bb, st, undo_bb, undo_st, mls, scores, sc, fc, ply):
    _check_time(sc, fc)
    sc[SC_NODES] += 1
    ...
    _sort_captures(bb, st, mls[ply], scores[ply], cnt)
...
@njit(cache=False, fastmath=True)
def _beta_cutoff_update(mv, depth, ply, killers, main_hist, quiets, quiet_cnt):
    if mv & (1 << 20):
        return
    if killers[0, ply] != mv:
...
@njit(cache=False, fastmath=True)
def _node_prologue(alpha, beta, depth, ply, rep_idx, pv_node, bb, st, rep,
                   static_evals, tt_key, tt_data, sc, fc):
    """Every non-recursive early exit of negamax, in BTC's order: draw rules,
    mate distance pruning, TT probe, leaf dispatch, in-check extension,
    static eval, improving, RFP. Returns
    (code, value, alpha, beta, depth, tt_move, in_check)."""
    if ply and (_is_repetition(bb, rep, rep_idx) or st[FIFTY] >= 100):
        return NODE_RETURN, 0, alpha, beta, depth, 0, 0
    ...
    if USE_MDP and ply:
        ...
        if alpha >= beta:
            return NODE_RETURN, alpha, alpha, beta, depth, 0, 0

    score, tt_move = tt_probe(bb, alpha, beta, depth, ply, tt_key, tt_data)
    if ply and not pv_node and score != NO_HASH_ENTRY:
        return NODE_RETURN, score, alpha, beta, depth, tt_move, 0

    _check_time(sc, fc)

    if depth == 0:
        return NODE_QSEARCH, 0, alpha, beta, depth, tt_move, 0
    if ply > MAX_SEARCH_PLY - 1:
        return NODE_RETURN, evaluate(bb, st), alpha, beta, depth, tt_move, 0
    ...
    if USE_RFP and depth < 3 and not pv_node and not in_check \
            and abs(beta) < MATE_SCORE:
        margin = 168 * (depth - improving)
        if ev - margin >= beta:
            return NODE_RETURN, ev - margin, alpha, beta, depth, tt_move, in_check

    return NODE_CONTINUE, 0, alpha, beta, depth, tt_move, in_check


@njit(cache=False, fastmath=True)
def negamax(alpha, beta, depth, ply, rep_idx, bb, st, undo_bb, undo_st, mls,
            scores, killers, main_hist, static_evals, pv_table, pv_len, rep,
            tt_key, tt_data, sc, fc):
    pv_len[ply] = ply
    pv_node = beta - alpha > 1

    code, value, alpha, beta, depth, tt_move, in_check = _node_prologue(
        alpha, beta, depth, ply, rep_idx, pv_node, bb, st, rep, static_evals,
        tt_key, tt_data, sc, fc)
    if code == NODE_RETURN:
        return value
    ...
    if USE_NULL and depth >= 3 and not in_check and ply:
        ...
        score = -negamax(-beta, -beta + 1, depth - 1 - 2, ply + 1,
                         ...
                         killers, main_hist, static_evals, pv_table, pv_len,
                         rep, tt_key, tt_data, sc, fc)
    ...
    for i in range(cnt):
        ...
        if moves_searched == 0:
            score = -negamax(-beta, -alpha, depth - 1, ply + 1, rep_idx + 1,
                             ...
                             main_hist, static_evals, pv_table, pv_len, rep,
                             tt_key, tt_data, sc, fc)
        else:
            score = -negamax(-alpha - 1, -alpha, depth - 1, ply + 1,
                             rep_idx + 1, bb, st, undo_bb, undo_st, mls,
                             scores, killers, main_hist, static_evals,
        ...
        if not (mv & (1 << 20)) and quiet_cnt < 256:
            quiets[quiet_cnt] = mv
            quiet_cnt += 1
        moves_searched += 1
        ...
        if score > alpha:
            ...
            if score >= beta:
                ...
                _beta_cutoff_update(mv, depth, ply, killers, main_hist,
                                    quiets, quiet_cnt)
                return beta
...
def _root_negamax(state, bb, st, alpha, beta, depth, rep_base):
    return negamax(alpha, beta, depth, 0, rep_base, bb, st, state.undo_bb,
                   ...
                   state.main_hist, state.static_evals, state.pv_table,
                   state.pv_len, state.rep, state.tt_key, state.tt_data,
                   state.sc, state.fc)
