...
def _engine_move(state, fen, depth):
    ...
    mv, _, _, _ = se.search_position(state, bb, st, keys, 1, 0, 600000,
                                     max_depth=depth)
...
def convert(name, fen, limit_moves, depth=10):
    ...
    while moves_played < limit_moves:
        ...
        uci = _engine_move(state, board.fen(), depth)
...
def main():
    ...
    print(f"warmup {time.perf_counter() - t0:.0f}s  "
          f"endgames={ev.USE_ENDGAMES}  threats={ev.USE_THREATS}", flush=True)
