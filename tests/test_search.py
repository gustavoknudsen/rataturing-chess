...
def _ref_negamax(alpha, beta, depth, ply, rep_idx, bb, st, undo_bb, undo_st,
    ...
    if ply and (se._is_repetition(bb, rep, rep_idx, 0, 0) or st[core.FIFTY] >= 100):
        return 0
    if depth == 0:
...
def _searched_value(state, fen, depth):
    ...
    # The accumulator is seeded once at the root; every deeper ply derives
    # from its parent. Without this, evaluate_cached reads a zeroed
    # accumulator and the differential fails for the wrong reason.
    if ev.USE_NNUE:
        btc_nnue.refresh(bb, ev.NET_FT_W, ev.NET_FT_B, ev.NET_L1,
                         state.acc[0], ev.NET_TABLE, ev.NET_BUCKETS)
    return se.negamax(state.acc, -se.INFINITY, se.INFINITY, depth, 0, 1, bb, st,
                      state.undo_bb, state.undo_st, state.mls, state.scores,
                      state.killers, state.main_hist, state.cap_hist,
                      state.corr, state.cont_hist, state.counters, state.played,
