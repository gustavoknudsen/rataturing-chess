...
Rated games start from a curated set that is not published, played **up to the
12th ply (move 6)**; the eight in the starter harness are a published sample.
Testing only on those eight makes results correlate once a match passes sixteen
games, and tunes the engine to eight positions.

**Prefer `--pgn`.** The random-walk generator below produces positions that are
balanced and diverse but not *realistic*: six to twelve random legal plies
gives lines like 1.a3 f5 2.b4 g6 3.g3 h4, which no opponent will ever play.
Measuring on those risks tuning for positions the engine will never see. The
PGN path instead takes real opening theory from a standard engine-testing book
(`8moves_v3.pgn`, eight moves deep) truncated to 12 plies, which is exactly
the tournament's stated depth.

Run directly to (re)generate the cache:
    python openings.py --pgn <path-to-8moves_v3.pgn> [count]   # preferred
    python openings.py [count]                                 # random walks
"""
...
def from_pgn(path, count, plies=12):
    """Real opening lines truncated to `plies`, deduplicated.

    The tournament plays curated openings to the 12th ply, so a book of actual
    theory at that depth is the faithful test distribution. No balance filter
    is applied: these are published opening lines, already reasonable by
    construction, and filtering them by our own evaluation would select for
    positions our evaluation happens to like."""
    import chess.pgn
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == "--pgn":
        path = sys.argv[2]
        count = int(sys.argv[3]) if len(sys.argv) > 3 else 600
        fens = from_pgn(path, count)
    else:
